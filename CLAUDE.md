...
3. **格式檢查**：「🔥 格式檢查要求」中的所有檢查項目
   1. **第一步**：立即建立 TODO 清單，列出「🔥 格式檢查要求」的「2. 語言與排版規範（僅針對 git 異動部分）」、「3. markdown格式檢查（僅針對 git 異動部分）」檢查項目
   2. **第二步**：逐項執行格式檢查
   3. **禁止**：不可跳過或延後「🔥 格式檢查要求」
...
- [ ] **5. 購票及預定等整理**：使用 Grep 搜尋 todo.md 相關項目（購票、預訂），避免完整讀取文件
...
- [ ] **1. 資訊查證：** 所有 Google Maps 連結、時間、營業時間已標註「✅ 已查證...」或「⚠️ 需確認...」
- [ ] **2. 語言與排版規範（僅針對 git 異動部分）：** 僅檢查本次新增或修改的文字內容，不重新檢查整個文件
- [ ] **3. markdown格式檢查（僅針對 git 異動部分）：** 僅對修改的行執行 markdownlint，使用 `git diff --name-only` 確認異動檔案
...
- ✅ **必須使用**：
  ...
  - **交通資訊：** 路線、時間、轉乘
    - 所有交通時間以 **Google Maps** 實際查詢為準，標準流程如下：
      1. **優先使用 WebSearch** 搜尋「google maps 路線規劃 [起點] [終點]」（比 WebFetch 更精準）
