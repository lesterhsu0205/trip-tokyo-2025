...
1. **行程內容編排**：
   - **思考順序**：資訊查詢 > 時間合理性 > 交通銜接
2. **行程內容檢查**：[🔥 行程內容檢查要求](#-行程內容檢查要求) 中的所有檢查項目
   - **第一步**：立即建立 TODO 清單，列出 [🔥 行程內容檢查要求](#-行程內容檢查要求) 所有檢查項目
   - **第二步**：逐項執行 TODO 清單
   - **禁止**：不可跳過或延後 [🔥 行程內容檢查要求](#-行程內容檢查要求)
3. **格式檢查**：[🔥 格式檢查要求](#-格式檢查要求) 中的所有檢查項目
...
- [ ] **1. 交通資訊正確性**：針對當日全部行程的下列資訊執行 WebFetch 以取得最新資料用以比對驗證
      - [Google Maps URL](./2025/tokyo/CLAUDE.md#google-maps-url)
      - [Yahoo Transit URL](./2025/tokyo/CLAUDE.md#yahoo-路線-url)
- [ ] **2. 遞迴檢查當日行程時序**：依序檢查下列 4 個子項目，遇到錯誤立即修正，修正完後再次從頭依序檢查，直到所有項目一口氣全通過
...
- [ ] **3. 購票及預定等整理**：使用 Grep 搜尋 todo.md 相關項目（購票、預訂），避免完整讀取文件
- [ ] **4. 購物地點及其他行程中要點整理**：使用 Grep 搜尋 remark.md 相關項目，避免完整讀取文件

**⚠️ 共計 4 項主要檢查類別（含 4 個時序子項目），執行前必須先建立完整 TODO 清單**
