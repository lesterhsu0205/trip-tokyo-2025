...
- [ ] **1. 交通資訊正確性**
  - 針對當日全部行程執行 WebFetch 取得最新資料比對驗證：
...
- [ ] **2. 遞迴檢查當日行程時序**
  - 依序檢查下列 4 個子項目，遇到錯誤立即修正，修正完後再次從頭依序檢查，直到所有項目一口氣全通過：
...
1. 立即暫停當前任務
2. 修正錯誤
3. **修正後重新檢查**：修正錯誤後，必須從發生錯誤的步驟重新開始檢查
   - **範例**：檢查項目 1→2→3，若第 2 項發現錯誤
   - **處理**：立即修正第 2 項 → 重新執行第 2 項檢查 → 確認通過後繼續第 3 項
   - **原則**：確保修正後的內容符合所有後續檢查要求
